...
const branchPageElementSizeX = unsafe.Sizeof(branchPageElementX{})
...
func (p *page) branchPageElementX(index uint16) *branchPageElementX {
	off := uintptr(index) * branchPageElementSizeX
	return (*branchPageElementX)(unsafe.Pointer(uintptr(unsafe.Pointer(p)) + pageHeaderSize + off))
}
...
func (p *page) branchPageElementsX() []branchPageElementX {
	...
	return *(*[]branchPageElementX)(unsafe.Pointer(&reflect.SliceHeader{
		Data: uintptr(unsafe.Pointer(p)) + pageHeaderSize,
		Len:  int(p.count),
		Cap:  int(p.count),
	}))
